	...
	<div>
		<ul>
			<li v-for="item in this.events">{{item}}
				...
				<a @click="showModalEvent(item._id, item.event_name, item.date, item.location,item.description, item.event_code, item.theme)">
		...
		<l-map @click="addMarker" style="height: 300px; width: 300px":zoom="zoom" :center="center" @update:zoom="zoomUpdated" @update:center="centerUpdated" @update:bounds="boundsUpdated">
		...
		<div class="field">
		  ...
		  <div class="control">
		    <input class="input" type="text" v-model="event_code" placeholder="Code pour accéder à la soirée">
		  </div>
		</div>
		...
		<div class="modal" v-bind:class="{'is-active' : isActiveEvent }">
			  ...
			  <div class="modal-content">
				...
				<l-map @click="addMarker_2" style="height: 300px; width: 300px":zoom="zoom_2" :center="center_2" @update:zoom="zoomUpdated_2" @update:center="centerUpdated_2" @update:bounds="boundsUpdated_2">
					<l-tile-layer :url="url_2"></l-tile-layer>
					<l-marker v-for="marker, index in markers_2" :lat-lng="marker" ></l-marker>
				</l-map>
				...
				<div class="field">
				  <label class="label">Code d'accès</label>
				  <div class="control">
				    <input class="input" type="text" v-model="tempCode" placeholder="Code pour accéder à la soirée">
				  </div>
				</div>
	...
	export default{
		...
		methods :{
			...
			submit(){
				...
				axios.post('parties/create',eventData).then(response => console.log(response.data))
			...
			showModalEvent(id, name, date, location, description, code, theme){
				setTimeout(function() { window.dispatchEvent(new Event('resize')) }, 250); // Aide à charger correctement la map (hack)
